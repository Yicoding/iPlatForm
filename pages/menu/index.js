...
Page({
  ...
  async saveBtn(e) {
    ...
    try {
      ...
      const { data } = await ajax({
        url: config.service.updateGoodsType,
        method: 'PUT',
        data: {
          id,
          name
      ...
      console.log('updateGoodsType', data);
      const dataList = JSON.parse(JSON.stringify(this.data.dataList));
      dataList[index].name = name;
      dataList[index].edit = false;
      this.setData({ dataList });
    } catch (e) {
      console.log('updateGoodsType报错', e);
    } finally {
      wx.hideLoading();
    }
  },
  ...
  async addType() {
    try {
      ...
      const { userInfo } = this.data;
      const { data } = await ajax({
        url: config.service.addGoodsType,
        method: 'POST',
        data: {
          name,
  ...
  remove(e) {
    ...
    wx.showModal({
      ...
      success: (res) => {
        ...
        const { confirm } = res;
        if (confirm) { // 确定
          this.removeGoodsType(item.id, index);
        }
      }
    })
  },
  // 删除单个
  async removeGoodsType(id, index) {
    wx.showLoading({
      title: '加载中...',
      mask: true
    });
    try {
      ...
      console.log('removeGoodsType', data);
      const dataList = JSON.parse(JSON.stringify(this.data.dataList));
      ...
      console.log('removeGoodsType报错', e);
    } finally {
      wx.hideLoading();
    }
  },
